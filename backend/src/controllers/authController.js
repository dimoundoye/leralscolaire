...
const { startSession, clearSession } = require('../utils/session');
...
const authController = {
  ...
  async login(req, res, next) {
    ...
    try {
      ...
      // 5. Détecter si le professeur est actuellement Président de Jury BAC actif et NON EXPIRÉ
  ...
  /**
   * Session en cours : informations de l'utilisateur connecté (via le cookie)
   */
  async me(req, res) {
    return res.json({ user: req.user.user || { id: req.user.id, role: req.user.role } });
  },
