...
const documentController = {
  ...
  async downloadBulletin(req, res) {
    ...
    try {
      if (req.user.role === 'ELEVE') {
        ...
        if (eleveCheck.rows.length === 0 || eleveCheck.rows[0].user_id !== req.user.id) {
          return res
            .status(403)
            .json({ message: 'Accès refusé. Vous ne pouvez télécharger que vos propres bulletins.' });
        }
      }
  ...
  // GET /attestation/:eleveId
  async downloadAttestation(req, res) {
    const { eleveId } = req.params;
    try {
      ...
      if (req.user.role === 'ELEVE') {
        ...
        if (eleveCheck.rows.length === 0 || eleveCheck.rows[0].user_id !== req.user.id) {
          return res
            .status(403)
            .json({ message: 'Accès refusé. Vous ne pouvez demander que vos propres attestations.' });
        }
      } else {
    ...
    `,
      ...
      doc
        .fontSize(15)
        .font('Helvetica-Bold')
        .text("ATTESTATION D'INSCRIPTION", 50, 130, { align: 'center', width: 500 });
  ...
  // GET /dossier-transfert/:eleveId
  async downloadDossierTransfert(req, res) {
    const { eleveId } = req.params;
    try {
      if (req.user.role === 'ELEVE') {
        const eleveCheck = await db.query('SELECT user_id FROM eleves WHERE id = $1', [eleveId]);
  ...
  async listAttestationRequests(req, res) {
    try {
  ...
  // POST /attestations/requests/:id/accept
  async acceptAttestationRequest(req, res) {
    const { id } = req.params;
    try {
      const etabRes = await db.query('SELECT id FROM etablissements WHERE admin_id = $1', [req.user.id]);
      if (etabRes.rows.length === 0) {
  ...
  // POST /attestations/requests/:id/refuse
  async refuseAttestationRequest(req, res) {
    const { id } = req.params;
  ...
  // GET /classe/:classeId/classement-pdf
  async downloadClassementPdf(req, res) {
