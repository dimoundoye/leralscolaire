...
const juryBacController = {
  // GET /my-jury
  async getMyJury(req, res) {
    try {
      ...
      if (!jury) {
        return res.status(404).json({ message: 'Aucun jury attribué.' });
      }

      // Le hash du mot de passe temporaire ne quitte jamais le serveur
      const { password_hash: _hash, ...juryPublic } = jury;
      res.json(juryPublic);
    } catch (err) {
      console.error(err);
  ...
  // GET /candidats/:id/notes
  async getCandidatNotes(req, res) {
    try {
  ...
  // POST /candidats/:id/notes
  async saveCandidatNotes(req, res) {
  ...
  // POST /candidats/:id/repecher
  async repecherCandidat(req, res) {
  ...
  // PUT /publier
  async publierResultatsJury(req, res) {
    try {
      const juryNom = req.juryInfo ? req.juryInfo.numero_jury : req.body.numero_jury;
      if (!juryNom) return res.status(400).json({ message: 'Numéro de jury requis.' });
...
module.exports = juryBacController;
