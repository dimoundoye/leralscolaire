...
const elevePortalController = {
  ...
  // GET /notes-evolution
  async getNotesEvolution(req, res) {
    try {
      const eleveId = await getEleveId(req.user.id);
  ...
  async getAttestationHistory(req, res) {
    try {
      const eleveId = await getEleveId(req.user.id);
      const result = await db.query(
...
module.exports = elevePortalController;
