...
async function generateDossierScolairePdf(eleveData, signalementsList, etablissementData, res) {
  ...
  res.setHeader(
    'Content-Disposition',
    `attachment; filename="Dossier_Scolaire_${eleveData.nom}_${eleveData.prenom}.pdf"`
  );
  doc.pipe(res);
  ...
  doc
    ...
    .fontSize(16)
    .fillColor('#0f172a')
    .text(etablissementData?.nom || 'LERALSCOLAIRE - ÉTABLISSEMENT SCOLAIRE', { align: 'center' });
  if (etablissementData?.region || etablissementData?.ville) {
    doc
      ...
      .fillColor('#64748b')
      .text(
        `${etablissementData.ville || ''} ${etablissementData.region ? '(' + etablissementData.region + ')' : ''}`,
        { align: 'center' }
      );
  }
  doc.moveDown(1);
  ...
  doc
    ...
    .font('Helvetica-Bold')
    .fontSize(14)
    .text('DOSSIER SCOLAIRE INDIVIDUEL', 50, doc.y - 23, { align: 'center' });
  doc.moveDown(1.5);
  ...
  doc
    .text(`Nom : `, 55, startY + 12, { continued: true })
    .font('Helvetica')
    .text(`${eleveData.nom || 'N/A'}`);
  doc
    .font('Helvetica-Bold')
    .text(`Prénom : `, 55, startY + 28, { continued: true })
    .font('Helvetica')
    .text(`${eleveData.prenom || 'N/A'}`);
  doc
    .font('Helvetica-Bold')
    .text(`Identifiant National : `, 55, startY + 44, { continued: true })
    .font('Helvetica')
    .text(`${eleveData.identifiant_national || eleveData.ine || 'N/A'}`);
  doc
    .font('Helvetica-Bold')
    .text(`Date de Naissance : `, 55, startY + 60, { continued: true })
    .font('Helvetica')
    .text(`${eleveData.date_naissance ? new Date(eleveData.date_naissance).toLocaleDateString('fr-FR') : 'N/A'}`);

  doc
    .font('Helvetica-Bold')
    .text(`Sexe : `, 310, startY + 12, { continued: true })
    .font('Helvetica')
    .text(`${eleveData.sexe || 'N/A'}`);
  doc
    .font('Helvetica-Bold')
    .text(`Classe Actuelle : `, 310, startY + 28, { continued: true })
    .font('Helvetica')
    .text(`${eleveData.classe_nom || 'Non attribuée'}`);
  doc
    .font('Helvetica-Bold')
    .text(`Téléphone Parent : `, 310, startY + 44, { continued: true })
    .font('Helvetica')
    .text(`${eleveData.telephone_parent || 'N/A'}`);
  doc
    .font('Helvetica-Bold')
    .text(`Adresse Parent : `, 310, startY + 60, { continued: true })
    .font('Helvetica')
    .text(`${eleveData.adresse_parent || 'N/A'}`);
  ...
  doc
    ...
    .fontSize(12)
    .fillColor('#0f172a')
    .text('2. REGISTRE DISCIPLINAIRE & REMARQUES PÉDAGOGIQUES');
  doc.moveDown(0.5);
  ...
  if (!signalementsList || signalementsList.length === 0) {
    ...
    doc
      ...
      .font('Helvetica')
      .fontSize(10)
      .text('Aucune remarque négative ni convocation enregistrée. Dossier disciplinaire vierge.', 55, doc.y - 20);
    doc.moveDown(1.5);
  } else {
    signalementsList.forEach((item) => {
      ...
      const auteurStr =
        item.auteur_nom_complet || (item.matiere_nom ? `Professeur de ${item.matiere_nom}` : 'Administration');
      ...
      doc
        ...
        .fillColor('#475569')
        .text(
          `Date : ${dateStr} | Émetteur : ${auteurStr} ${item.statut ? '| Statut : ' + item.statut : ''}`,
          50,
          itemY + 22
        );
