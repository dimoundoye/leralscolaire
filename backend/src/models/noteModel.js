...
const NoteModel = {
  ...
  async createMatiere(nom, code) {
    ...
    const { rows } = await db.query('INSERT INTO matieres (nom, code_matiere) VALUES ($1, $2) RETURNING *', [
      nom,
      codeMatiere,
    ]);
    return rows[0];
  ...
  async updateMatiere(id, nom, code) {
    const { rows } = await db.query('UPDATE matieres SET nom = $1, code_matiere = $2 WHERE id = $3 RETURNING *', [
      nom,
      code,
      id,
    ]);
  ...
  async insertNote(data, client = db) {
    const { eleveId, matiereId, valeur, appreciation, semestre, trimestre, typeNote, professeurId, etablissementId } =
      data;
  ...
  async saveBulletinDecision(
    ...
    `,
      [eleveId, classeId, anneeScolaire || '', moyenneGenerale, decision, decisionDetail, decisionAuto !== false]
    );
    return true;
  },
};
