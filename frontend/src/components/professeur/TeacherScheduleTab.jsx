        ...
        <div style={{ display: 'flex', borderBottom: '1px solid #e2e8f0', marginBottom: '20px', gap: '16px' }}>
          <button
            ...
            style={{
              ...
              fontWeight: scheduleViewMode === 'weekly' ? 700 : 500,
              fontSize: '13px',
              cursor: 'pointer',
            }}
          >
          ...
          <button
            ...
            style={{
              ...
              fontWeight: scheduleViewMode === 'etablissement' ? 700 : 500,
              fontSize: '13px',
              cursor: 'pointer',
            }}
          >
                  ...
                  return sortedSlots.map((slotKey) => (
                    <tr key={slotKey}>
                      {['Lundi', 'Mardi', 'Mercredi', 'Jeudi', 'Vendredi', 'Samedi'].map((jour) => {
                        const dayEntries = activeSchedule.filter(
                          (e) =>
                            e.jour_semaine === jour &&
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', height: '100%' }}>
                                {dayEntries.map((slot) => {
                                  const conflict = hasConflict(
                                    slot,
                                    schedule.filter((s) => s.jour_semaine === jour)
                                  );
                                      ...
                                      <div>
                                        <div
                                          style={{
                                            fontSize: '10px',
                                            fontWeight: 800,
                                            color: colors.text || 'var(--slate-500)',
                                            marginBottom: '3px',
                                          }}
                                        >
                                          {slotKey}
                                        ...
                                        <div
                                          style={{
                                            fontSize: '10px',
                                            color: colors.text || 'var(--slate-600)',
                                            fontWeight: 600,
                                          }}
                                        >
          ...
          <div>
            {Array.from(new Set(schedule.map((s) => s.etablissement_nom))).length === 0 ? (
              ...
              Array.from(new Set(schedule.map((s) => s.etablissement_nom))).map((etabName) => {
                const etabSlots = schedule
                  .filter((s) => s.etablissement_nom === etabName)
                  .sort((a, b) => {
                    const daysOrder = {
                      Lundi: 1,
                      Mardi: 2,
                        ...
                        <tbody>
                          {etabSlots.map((slot) => (
                            <tr key={slot.id} style={{ borderBottom: '1px solid #edf2f7' }}>
                              ...
                              <td style={{ padding: '10px 18px', color: colors.text, fontWeight: 700 }}>
                              ...
                              <td style={{ padding: '10px 18px', color: 'var(--text-slate-500)' }}>
                                {slot.salle || '—'}
                              </td>
                            </tr>
                          ))}
