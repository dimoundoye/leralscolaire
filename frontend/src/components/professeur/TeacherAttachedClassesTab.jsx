...
const TeacherAttachedClassesTab = ({
  ...
  const yearDashClasses = (dashboardDetails?.classes || []).filter(
    (cls) => !cls.annee_scolaire || cls.annee_scolaire === profAnneeFilter
  );
              ...
              {filtered.length > 0 ? (
                filtered.map((cls) => (
                  <tr key={`${cls.classe_id}-${cls.matiere_id}`} style={{ borderBottom: '1px solid #edf2f7' }}>
                    <td style={{ padding: '12px', fontWeight: 600, color: 'var(--text-slate-700)' }}>
                      {cls.etablissement_nom}
                    </td>
                    <td style={{ padding: '12px', fontWeight: 700 }}>
                      {cls.classe_nom} ({cls.niveau})
                    </td>
                    <td style={{ padding: '12px', fontWeight: 600 }}>{cls.matiere_nom}</td>
                    <td style={{ padding: '12px', textAlign: 'center', fontWeight: 700, color: 'var(--primary-blue)' }}>
                      {cls.total_students}
                    </td>
                        ...
                        {['S1', 'S2'].map((term) => {
                          ...
                          return (
                            <div key={term} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                              <span style={{ fontSize: '11px', fontWeight: 700, color: 'var(--text-slate-500)' }}>
                                {term}:
                              </span>
                              <span
                                style={{
