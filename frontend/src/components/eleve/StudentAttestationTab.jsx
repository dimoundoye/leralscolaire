...
const StudentAttestationTab = ({
  ...
  const currentClassHistory = attestationHistory.filter(
    (dem) => !dem.classe_id || dem.classe_id === profile?.classe_id
  );
      ...
      <div className="card-box">
        ...
        >
          ...
          <div>
            ...
            <p style={{ margin: 0, fontSize: '12px', color: 'var(--text-muted)' }}>
              Soumettez une demande à votre établissement. Le document officiel vous sera remis une fois la demande
              traitée.
            </p>
          </div>
        </div>
            ...
            if (latest.statut === 'EN_ATTENTE')
              return (
                ...
                >
                  ...
                  <div>
                    <div style={{ fontWeight: 800, fontSize: '13px', color: '#78350f', marginBottom: '4px' }}>
                      Demande en cours de traitement
                    </div>
                    <div style={{ fontSize: '12px', color: '#92400e', lineHeight: 1.5 }}>
                      Votre établissement a bien reçu votre demande et la traitera prochainement. Vous serez notifié dès
                      qu'une décision est prise.
                    </div>
                  </div>
                </div>
              );
            if (latest.statut === 'ACCEPTE') {
              if (latest.deja_telecharge) {
                const downloadDateStr = latest.date_telechargement
                  ? new Date(latest.date_telechargement).toLocaleDateString('fr-SN', {
                      day: 'numeric',
                      month: 'long',
                      year: 'numeric',
                      hour: '2-digit',
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      <div style={{ fontWeight: 800, fontSize: '13px', color: '#334155', marginBottom: '4px' }}>
                        Attestation déjà téléchargée
                      </div>
                      <div style={{ fontSize: '12px', color: '#475569', lineHeight: 1.5 }}>
                        Votre attestation d'inscription officielle pour la classe{' '}
                        <strong>{profile?.classe_nom || 'actuelle'}</strong> a été téléchargée le {downloadDateStr}.
                        <br />
                        Conformément à la réglementation scolaire, ce document est strictement obligatoire et délivré{' '}
                        <strong>une seule fois</strong>. Si vous l'avez égaré, veuillez vous adresser directement à
                ...
                >
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '12px', color: '#b91c1c', lineHeight: 1.5 }}>
                      Motif : {latest.motif_refus || 'Aucun motif fourni'}. Vous pouvez soumettre une nouvelle demande
                      ci-dessous.
                    </div>
                  </div>
                </div>
        ...
        {(currentClassHistory.length === 0 || currentClassHistory[0]?.statut === 'REFUSE') && (
          ...
          >
            ...
            <form
              onSubmit={handleSubmitAttestationRequest}
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <button
                  ...
                  style={{
                    ...
                    fontSize: '13px',
                    fontWeight: 700,
                    cursor: attestationSubmitting ? 'not-allowed' : 'pointer',
                    opacity: attestationSubmitting ? 0.7 : 1,
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {attestationHistory.map((dem) => {
                const dateStr = new Date(dem.date_demande).toLocaleDateString('fr-SN', {
                  day: 'numeric',
                  month: 'long',
                  year: 'numeric',
                });
                    ...
                    <div>
                      ...
                      {dem.motif_demande && (
                        <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '2px' }}>
                          Motif : {dem.motif_demande}
                        </div>
                      )}
                      {dem.statut === 'REFUSE' && dem.motif_refus && (
                        <div style={{ fontSize: '11px', color: '#dc2626', marginTop: '2px' }}>
                          Refus : {dem.motif_refus}
                        </div>
                      )}
                    </div>
                    <span
