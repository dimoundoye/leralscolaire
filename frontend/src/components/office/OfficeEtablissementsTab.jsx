    ...
    <div className="ob-tab-pane">
      <div className="ob-page-header">
        <div>
          ...
          <p>
            Gestion des lycées et centres d'examen nationaux avec génération automatique des comptes administrateurs
          </p>
        </div>
        ...
        <table className="ob-table">
          ...
          <tbody>
            {etablissements.map((e) => (
              <tr key={e.id}>
                <td>
                  <code className="ob-id-code">{e.code_etablissement}</code>
                </td>
                <td>
                  <strong style={{ color: 'var(--primary-color)' }}>{e.nom}</strong>
                  {e.autorisation_numero && (
                    <div style={{ fontSize: 11, color: '#64748b' }}>Arrêté: {e.autorisation_numero}</div>
                  )}
                </td>
                <td>
                  {e.region} ({e.ville})
                ...
                <td>
                  <span className="ob-statut-pill" style={{ background: '#eff6ff', color: '#1d4ed8' }}>
                    {e.total_eleves || 0} Élèves
                  </span>
                </td>
                <td>
                  <span className="ob-statut-pill" style={{ background: '#f0fdf4', color: '#15803d' }}>
                    {e.total_profs || 0} Enseignants
                  </span>
                </td>
              </tr>
            ))}
