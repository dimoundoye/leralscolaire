...
export const OfficeMessagerieTab = ({
  ...
  const filteredMsgs = messagesList.filter((m) => {
    if (selectedChannelCategory === 'PROFESSEUR')
      return (
        m.destinataire_type === 'PROFESSEUR' ||
        m.destinataire_type === 'ALL_PROFESSEURS' ||
        m.expediteur_role === 'PROFESSEUR'
      );
    if (selectedChannelCategory === 'ETABLISSEMENT')
      return (
        m.destinataire_type === 'ADMIN_ETABLISSEMENT' ||
        m.destinataire_type === 'ALL_ETABLISSEMENTS' ||
        m.expediteur_role === 'ADMIN_ETABLISSEMENT'
      );
  ...
  return (
    <div className="ob-tab-pane">
      <div className="ob-page-header">
        ...
        <button className="ob-btn ob-btn-ghost" onClick={fetchMessagesOffice}>
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
            {[
              ...
              {
                id: 'ETABLISSEMENT',
                label: ' Établissements (Proviseurs)',
                icon: <School size={16} />,
                color: '#131e6c',
              },
              ...
              <button
                ...
                key={cat.id}
                onClick={() => {
                  setSelectedChannelCategory(cat.id);
                  setNewMsgForm((f) => ({ ...f, destinataire_type: cat.id }));
                }}
                style={{
                  display: 'flex',
                  alignItems: 'center',
          ...
          <form
            onSubmit={handleSendOfficeMessage}
            style={{ background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: 12, padding: '14px' }}
          >
            <div
              style={{
                fontSize: 12,
                ...
                >
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <strong style={{ fontSize: 13, color: '#131e6c' }}>
                        {m.expediteur_nom_complet || m.expediteur_nom}
                      </strong>
                  ...
                  <div style={{ fontSize: 12, color: '#334155', whiteSpace: 'pre-line', lineHeight: 1.5 }}>
                    {m.contenu}
                  </div>
                </div>
              ))}
