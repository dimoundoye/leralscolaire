    ...
    <div className="ob-tab-pane">
      <div className="ob-page-header">
        <div>
          ...
          <p>
            Gestion du corps enseignant et des examinateurs du BAC/BFEM avec identifiants et mot de passe temporaire
            unique
          </p>
        </div>
        ...
        <table className="ob-table" style={{ fontSize: 12 }}>
          ...
          <tbody>
            {professeursList.map((p) => (
              <tr key={p.id}>
                <td>
                  <code className="ob-id-code">{p.identifiant_national || 'PROF-SN'}</code>
                </td>
                <td>
                  <span
                    style={{
                ...
                <td>
                  ...
                  <div style={{ fontSize: 11, color: '#64748b' }}>
                    {p.telephone || '—'} {p.cni_numero ? `• CNI: ${p.cni_numero}` : ''}{' '}
                    {p.matricule_solde ? `• Solde: ${p.matricule_solde}` : ''}
                  </div>
                </td>
                <td>{p.email}</td>
                <td>
                  <span className="ob-badge-type">{p.matiere_principale || 'Général'}</span>
                </td>
