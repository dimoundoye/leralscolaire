    ...
    <div className="ob-tab-pane">
      <div className="ob-page-header">
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button className="ob-btn ob-btn-ghost" onClick={copyPublicLink}>
            <Copy size={15} /> Copier le Lien d'Inscription Publique
          </button>
          <button className="ob-btn ob-btn-ghost" onClick={fetchDemandes}>
      ...
      <div className="ob-stats-grid mb-6">
        ...
        <div className="ob-stat-card ob-stat-ajournes" style={{ background: '#fffbeb', borderColor: '#fde68a' }}>
          <div className="ob-stat-icon" style={{ background: '#fef3c7', color: '#b45309' }}>
            <Clock size={22} />
          </div>
          <div>
            <div className="ob-stat-num" style={{ color: '#b45309' }}>
              {totalEnAttente}
            </div>
        ...
        <div className="ob-stat-card" style={{ background: '#fef2f2', borderColor: '#fecaca' }}>
          <div className="ob-stat-icon" style={{ background: '#fee2e2', color: '#b91c1c' }}>
            <XCircle size={22} />
          </div>
          <div>
            <div className="ob-stat-num" style={{ color: '#b91c1c' }}>
              {totalRejetees}
            </div>
          ...
          <table className="ob-table" style={{ width: '100%', minWidth: '1150px', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {filteredDemandes.map((d) => {
                ...
                try {
                  docsObj =
                    typeof d.documents_fournis === 'string'
                      ? JSON.parse(d.documents_fournis || '{}')
                      : d.documents_fournis || {};
                } catch (e) {
                  docsObj = {};
                }
                  ...
                  <tr key={d.id}>
                    ...
                    <td>
                      ...
                      <div style={{ fontSize: 11, color: '#64748b' }}>
                        {d.type_demande === 'ETABLISSEMENT'
                          ? d.specialite_ou_code || 'Établissement Scolaire'
                          : d.specialite_ou_code || 'Professeur'}
                      </div>
                    </td>
                    <td>
                      <code>{d.email}</code>
                    ...
                    <td style={{ whiteSpace: 'nowrap' }}>
                      {d.region} ({d.ville || 'Dakar'})
                    </td>
                    <td>
                      <div style={{ fontSize: 11 }}>
                        {d.cni_numero && (
                          <div>
                            <strong>CNI:</strong> {d.cni_numero}
                          </div>
                        )}
                        {d.autorisation_numero && (
                          <div>
                            <strong>Arrêté:</strong> {d.autorisation_numero}
                          </div>
                        )}
                        {d.matricule_solde && (
                          <div>
                            <strong>Solde:</strong> {d.matricule_solde}
                          </div>
                        )}
                        {!d.cni_numero && !d.autorisation_numero && !d.matricule_solde && (
                      ...
                      {d.statut === 'EN_ATTENTE' ? (
                        <div className="ob-actions-row" style={{ display: 'flex', gap: 6 }}>
                          ...
                          <button
                            ...
                            style={{ whiteSpace: 'nowrap' }}
                            onClick={() => handleRejeterDemande(d)}
                          >
                            ✕ Rejeter
                          </button>
                        </div>
      ...
      {selectedDemandeForDocs &&
        (() => {
          ...
          try {
            docsObj =
              typeof selectedDemandeForDocs.documents_fournis === 'string'
          ...
          return (
            <div className="ob-modal-backdrop" onClick={() => setSelectedDemandeForDocs(null)}>
              <div className="ob-modal-box" style={{ maxWidth: 680 }} onClick={(e) => e.stopPropagation()}>
                <div
                  className="ob-modal-header"
                  style={{ background: '#131e6c', color: '#fff', borderRadius: '12px 12px 0 0', padding: '16px 20px' }}
                >
                  <div>
                    <h3
                      style={{
                    ...
                    <p style={{ margin: '4px 0 0', fontSize: 12, opacity: 0.85 }}>
                      {selectedDemandeForDocs.type_demande === 'ETABLISSEMENT'
                        ? 'Établissement Demandeur'
                        : 'Enseignant Demandeur'}{' '}
                      :{' '}
                      {selectedDemandeForDocs.prenom
                        ? `${selectedDemandeForDocs.prenom} ${selectedDemandeForDocs.nom}`
                        : selectedDemandeForDocs.nom}
                    </p>
                  </div>
                  <button
                    className="ob-modal-close"
                    onClick={() => setSelectedDemandeForDocs(null)}
                    style={{ color: '#fff' }}
                  >
                    ✕
                  </button>
                ...
                <div className="ob-modal-body" style={{ padding: '20px' }}>
                  ...
                  >
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, fontSize: 12 }}>
                      <div>
                        <strong>Demandeur :</strong> {selectedDemandeForDocs.nom} {selectedDemandeForDocs.prenom}
                      </div>
                      <div>
                        <strong>Email :</strong> {selectedDemandeForDocs.email}
                      </div>
                      <div>
                        <strong>Téléphone :</strong> {selectedDemandeForDocs.telephone || '—'}
                      </div>
                      <div>
                        <strong>Région / Ville :</strong> {selectedDemandeForDocs.region} (
                        {selectedDemandeForDocs.ville})
                      </div>
                      {selectedDemandeForDocs.cni_numero && (
                        <div>
                          <strong>N° CNI :</strong> {selectedDemandeForDocs.cni_numero}
                        </div>
                      )}
                      {selectedDemandeForDocs.autorisation_numero && (
                        <div>
                          <strong>N° Arrêté :</strong> {selectedDemandeForDocs.autorisation_numero}
                        </div>
                      )}
                      {selectedDemandeForDocs.matricule_solde && (
                        <div>
                          <strong>Matricule Solde :</strong> {selectedDemandeForDocs.matricule_solde}
                        </div>
                      )}
                    </div>
                  </div>
                      ...
                      return (
                        <div
                          ...
                          style={{
                            ...
                            border: `1px solid ${fileName ? '#bbf7d0' : '#cbd5e1'}`,
                            borderRadius: 10,
                            padding: '14px',
                          }}
                        >
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            <div>
                              ...
                              <div style={{ fontSize: 11, color: fileName ? '#166534' : '#94a3b8', marginTop: 2 }}>
                                {fileName
                                  ? `📄 Fichier : ${typeof fileName === 'object' ? fileName.name : fileName}`
                ...
                >
                  <button className="ob-btn ob-btn-ghost" onClick={() => setSelectedDemandeForDocs(null)}>
                    Fermer
                  </button>
                  ...
                  {selectedDemandeForDocs.statut === 'EN_ATTENTE' && (
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button
                        className="ob-btn ob-btn-danger ob-btn-sm"
                        onClick={() => {
                          handleRejeterDemande(selectedDemandeForDocs.id);
                      ...
                      <button
                        className="ob-btn ob-btn-primary ob-btn-sm"
                        onClick={() => {
                          handleValiderDemande(selectedDemandeForDocs.id);
                          setSelectedDemandeForDocs(null);
                        }}
                      >
