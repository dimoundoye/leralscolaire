  ...
  return (
    <div className="ob-tab-pane">
        ...
        <div className="ob-mid-row mt-6">
          <div className="ob-card">
            <h3 className="ob-card-title">
              <BarChart3 size={18} /> Répartition des Inscrits & Admis par Série
            </h3>
          ...
          <div className="ob-card">
            <h3 className="ob-card-title">
              <BarChart3 size={18} /> Distribution Globale des Résultats
            </h3>
            <div style={{ width: '100%', height: 250 }}>
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={[
                      ...
                      {
                        name: 'Ajournés / Exclus',
                        value: Math.max(0, (stats.publies || 0) - (stats.admis || 0) - (stats.rattrapage || 0)),
                      },
                    ].filter((d) => d.value > 0)}
                    cx="50%"
                    cy="50%"
      ...
      <div className="ob-mid-row mt-6">
        <div className="ob-card">
          <h3 className="ob-card-title">
            <Medal size={18} style={{ color: '#eab308' }} /> Palmarès de la Session
          </h3>
          {palmares.length > 0 ? (
                ...
                <tbody>
                  {palmares.map((p, idx) => (
                    <tr key={p.id}>
                      ...
                      <td>
                        <span className="ob-badge-type">{p.serie}</span>
                      </td>
                      <td>
                        <strong style={{ fontSize: 14, color: '#15803d' }}>
                          {parseFloat(p.moyenne).toFixed(2)}/20
                        </strong>
                      </td>
                      <td>
                        <span style={{ fontSize: 11, fontWeight: 700, color: getMentionColor(p.mention) }}>
                          {p.mention}
                        </span>
                      </td>
                    </tr>
            ...
            <div className="ob-empty" style={{ padding: 30 }}>
              <p>Aucun lauréat publié pour le moment.</p>
            </div>
          )}
        </div>
        ...
        <div className="ob-card">
          <h3 className="ob-card-title">
            <BarChart3 size={18} /> Résultats par Série
          </h3>
          {stats?.par_serie?.length > 0 ? (
            <div className="ob-series-grid" style={{ gridTemplateColumns: '1fr' }}>
              {stats.par_serie.map((s) => {
                ...
                return (
                  <div
                    key={s.serie}
                    className="ob-serie-item"
                    style={{ background: '#f8fafc', padding: 12, borderRadius: 10 }}
                  >
                    <div className="ob-serie-header">
                      <span className="ob-serie-name">Série {s.serie || 'N/A'}</span>
                      <span className="ob-serie-taux" style={{ color: taux >= 50 ? '#15803d' : '#b91c1c' }}>
                        {taux}%
                      </span>
                    </div>
                    ...
                    <div className="ob-serie-sub">
                      {s.admis} admis / {s.total} candidats
                    </div>
                  </div>
                );
              })}
            ...
            <div className="ob-empty" style={{ padding: 30 }}>
              <p>Données par série non disponibles.</p>
            </div>
          )}
        </div>
      </div>
