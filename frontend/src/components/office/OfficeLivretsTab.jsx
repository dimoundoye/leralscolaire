import { Plus, Send, RefreshCw, BookOpen, CheckCircle, Clock } from 'lucide-react';
    ...
    <div className="ob-tab-pane">
      <div className="ob-page-header">
        <div>
          ...
          <p>
            Consultation, suivi du dossier scolaire (Seconde, Première, Terminale) et certification pour le Jury de
            délibération
          </p>
        </div>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          ...
          <button
            className="ob-btn"
            onClick={handleRestituerLivrets}
            style={{
              background: '#7c3aed',
              color: '#ffffff',
              border: 'none',
          ...
          <button className="ob-btn ob-btn-ghost" onClick={fetchLivrets}>
      ...
      <div className="ob-table-wrap">
        {livretsList.length === 0 ? (
          <div className="ob-empty">
            <BookOpen size={48} />
            <p>Aucun livret scolaire transmis pour cette session.</p>
          </div>
        ) : (
                ...
                <tr key={l.id}>
                  <td>
                    <strong style={{ color: 'var(--primary-color)' }}>
                      {l.eleve_prenom} {l.eleve_nom}
                    </strong>
                    <div style={{ fontSize: 11, color: '#64748b' }}>
                      <code>{l.identifiant_national}</code>
                    </div>
                  </td>
                  <td>
                    <div>
                      <strong>{l.etablissement_nom || 'Lycée'}</strong>
                    </div>
                    <div style={{ fontSize: 11, color: '#64748b' }}>{l.region || 'Sénégal'}</div>
                  </td>
                  <td>
                    <span className="ob-badge-type">{l.serie}</span>
                  </td>
                  <td>
                    <strong>{l.moyenne_seconde ? parseFloat(l.moyenne_seconde).toFixed(2) + '/20' : '—'}</strong>
                  </td>
                  <td>
                    <strong>{l.moyenne_premiere ? parseFloat(l.moyenne_premiere).toFixed(2) + '/20' : '—'}</strong>
                  </td>
                  <td>
                    <strong style={{ color: '#15803d' }}>
                      {l.moyenne_terminale ? parseFloat(l.moyenne_terminale).toFixed(2) + '/20' : '—'}
                    </strong>
                  </td>
                  <td>
                    ...
                    <div className="ob-actions-row">
                      {l.statut_validation !== 'CONFORME' && (
                        <button
                          ...
                          onClick={() => handleValiderLivret(l.id, 'CONFORME')}
                        >
                          ✓ Certifier
                        </button>
                      )}
                      {l.statut_validation !== 'REJETÉ' && (
                        <button
                          ...
                          onClick={() => handleValiderLivret(l.id, 'REJETÉ')}
                        >
                          ✕ Rejeter
                        </button>
                      )}
