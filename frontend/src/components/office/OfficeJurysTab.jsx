    ...
    <div className="ob-tab-pane">
      <div className="ob-page-header">
        ...
        <div className="ob-page-header-right" style={{ display: 'flex', gap: 8 }}>
          <button className="ob-btn ob-btn-ghost" onClick={fetchJurys}>
            <RefreshCw size={15} /> Actualiser
          </button>
          <button
            className="ob-btn"
            onClick={handleDispatchAlphabetique}
            style={{
              background: '#0284c7',
              color: '#ffffff',
              border: 'none',
        ...
        {filteredJurys.length === 0 ? (
          <div className="ob-empty">
            ...
            {(jurySearchQuery || juryRegionFilter) && (
              <button
                className="ob-btn ob-btn-ghost mt-2"
                onClick={() => {
                  setJurySearchQuery('');
                  setJuryRegionFilter('');
                }}
            ...
            <tbody>
              {filteredJurys.map((j) => (
                <tr key={j.id || j.numero_jury}>
                  <td>
                    <code className="ob-id-code" style={{ fontSize: 13, fontWeight: 700 }}>
                      {j.numero_jury}
                    </code>
                  </td>
                  <td>
                    <strong>{j.centre_examen}</strong>
                  </td>
                  <td>
                    {j.region} <span style={{ fontSize: 11, color: '#64748b' }}>({j.zone_commune || 'Centre'})</span>
                  </td>
                  <td>
                    <span className="ob-badge-type">{j.series_autorisees || 'Toutes séries'}</span>
                  </td>
                  <td>
                    {j.president_jury ? (
                      <div>
                        ...
                        {j.identifiant_temporaire && (
                          <div style={{ fontSize: 11, color: '#475569', marginTop: 2 }}>
                            ...
                            {j.mot_de_passe_temporaire && (
                              <span>
                                {' '}
                                (MDP: <strong>{j.mot_de_passe_temporaire}</strong>)
                              </span>
                            )}
                          </div>
                        )}
                  ...
                  <td>
                    {(() => {
                      ...
                      return (
                        <div>
                          <span
                            className={`ob-badge ${isExpired ? 'ob-badge-ajourne' : 'ob-badge-admis'}`}
                          ...
                          {j.date_expiration_acces && (
                            ...
                            >
                              Lim: {new Date(j.date_expiration_acces).toLocaleDateString('fr-FR')}{' '}
                              {new Date(j.date_expiration_acces).toLocaleTimeString('fr-FR', {
                                hour: '2-digit',
                                minute: '2-digit',
                              })}
                            </div>
