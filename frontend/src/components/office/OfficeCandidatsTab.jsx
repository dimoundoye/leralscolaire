    ...
    <div className="ob-tab-pane">
      <div className="ob-page-header">
        ...
        <div className="ob-page-header-right" style={{ display: 'flex', gap: 8 }}>
          ...
          <button
            className="ob-btn ob-btn-ghost"
            onClick={handleGenererNumeros}
            title="Générer automatiquement les numéros de table"
          >
      ...
      >
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          >
            <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <Award size={16} style={{ color: '#2563eb' }} /> 1. Sélectionner le Jury &amp; Centre d'Examen Hôte (
              {jurysList.length} Jurys enregistrés) :
            </span>
            {jurysList.length > 6 && (
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
            <div style={{ flex: '1 1 320px', minWidth: 280 }}>
              ...
              >
                <option value="">
                  Tous les Jurys du Sénégal ({filters.annee}) — ({jurysList.length} Jurys)
                </option>
                {jurysList.map((j) => (
                  <option key={j.id || j.numero_jury} value={j.numero_jury}>
                    {j.numero_jury} — {j.centre_examen} ({j.region}){' '}
                    {j.president_jury ? `· Pr. ${j.president_jury}` : ''}
                  </option>
                ))}
              </select>
            </div>
                ...
                {jurysList.map((j) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        style={{
                          background: isSelected ? '#ffffff' : '#1e1b4b',
        ...
        {(() => {
          ...
          const availableSeries =
            selectedJuryObj &&
            selectedJuryObj.series_autorisees &&
            selectedJuryObj.series_autorisees !== 'Toutes séries'
          ...
          return (
            <div style={{ paddingTop: 14, borderTop: '1px dashed #e2e8f0' }}>
              ...
              >
                <BookOpen size={16} style={{ color: '#059669' }} /> 2. Filtrer par Série de ce Jury{' '}
                {filters.jury ? `(${filters.jury})` : ''} :
              </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 15, fontWeight: 800 }}>
              {filters.jury
                ? `${filters.jury} — ${jurysList.find((j) => j.numero_jury === filters.jury)?.centre_examen || "Centre d'Examen"}`
                : 'Tous les Jurys du Sénégal'}
      ...
      <div className="ob-filters-bar">
        <div className="ob-search-wrap">
          <Search size={16} />
          <input
            placeholder="Rechercher un candidat, N° table, IUP…"
        ...
        <select
          value={filters.type_candidat}
          onChange={(e) => setFilters((f) => ({ ...f, type_candidat: e.target.value }))}
        >
        ...
        <select
          value={filters.statut_dossier}
          onChange={(e) => setFilters((f) => ({ ...f, statut_dossier: e.target.value }))}
        >
        ...
        <button className="ob-btn ob-btn-ghost" onClick={fetchCandidats} title="Actualiser">
          <RefreshCw size={15} />
        </button>
      </div>
      ...
      <div className="ob-table-wrap">
        {loading ? (
          <div className="ob-loading">
            <Loader2 size={28} className="ob-spin" /> Chargement…
          </div>
        ) : candidats.length === 0 ? (
                  ...
                  <tr key={c.id}>
                      ...
                      <div className="ob-name-cell">
                        ...
                        <div>
                          ...
                          <div className="ob-etab">
                            ...
                            <code style={{ fontSize: 11 }}>{c.identifiant_national}</code>
                          </div>
                    ...
                    <td>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                        <span className="ob-badge-type" style={{ width: 'fit-content' }}>
                          {c.type_examen}
                        </span>
                    ...
                    <td>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                        ...
                        {c.statut_candidat === 'SUSPENDU' && c.motif_suspension && (
                          <span
                            style={{ fontSize: 10, color: '#b91c1c', fontStyle: 'italic', maxWidth: 140 }}
                            title={c.motif_suspension}
                          >
                    ...
                    <td>
                      <div className="ob-actions-row">
                        <button
                          className="ob-icon-btn ob-icon-edit"
                        ...
                        <button
                          ...
                          style={{ background: '#f1f5f9', color: '#131e6c' }}
                          title="Historique participations"
                          onClick={() => fetchHistorique(c.eleve_id)}
                        >
                          <History size={14} />
                        </button>
                        ...
                        {c.statut_candidat === 'SUSPENDU' ? (
                          <button
                            ...
                            onClick={() => handleAnnulerSuspension(c.id)}
                          >
                            <CheckCircle size={14} />
                          </button>
                        ) : (
