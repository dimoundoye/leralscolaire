        ...
        >
          <div>
            ...
            <h3 style={{ margin: '4px 0 0', fontSize: '17px', fontWeight: 900, color: '#131e6c' }}>
              Évaluation de{' '}
              {String(professeur?.civilite || professeur?.sexe || '').toUpperCase() === 'F' ||
              String(professeur?.sexe || '')
                .toUpperCase()
                .startsWith('FEM')
                ? 'Mme'
                : 'M.'}{' '}
              {professeur?.nom || "l'Enseignant"}
            </h3>
          </div>
        ...
        {submitted ? (
          ...
          <form onSubmit={handleSubmit}>
            ...
            {renderStarPicker(
              q1,
              setQ1,
              '1. Pédagogie & Clarté des Explications',
              'Le professeur explique-t-il clairement les notions complexes ?'
            )}
            {renderStarPicker(
              q2,
              setQ2,
              '2. Ponctualité & Respect des Horaires',
              'Le professeur est-il ponctuel et assidu à chaque cours ?'
            )}
            {renderStarPicker(
              q3,
              setQ3,
              '3. Écoute & Disponibilité',
              'Le professeur répond-il aux questions et aide-t-il les élèves en difficulté ?'
            )}
            {renderStarPicker(
              q4,
              setQ4,
              '4. Qualité des Corrections & Devoirs',
              'Les devoirs sont-ils corrigés dans des délais raisonnables ?'
            )}
            {renderStarPicker(
              q5,
              setQ5,
              '5. Climat & Ambiance de Classe',
              'Le climat en classe est-il propice au travail et au respect ?'
            )}
