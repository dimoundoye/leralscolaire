    ...
    <div className="partages-view">
      ...
      <div style={{ marginBottom: '20px' }}>
        <div className="status-toggle-pill" style={{ display: 'inline-flex' }}>
          <button
            className={`status-toggle-btn ${partageView === 'received' ? 'active' : ''}`}
            onClick={() => setPartageView('received')}
          >
            Reçus ({partagesRecus.length})
          </button>
          <button
            className={`status-toggle-btn ${partageView === 'sent' ? 'active' : ''}`}
            onClick={() => setPartageView('sent')}
          >
                  ...
                  <tr key={d.id} className={!d.lu ? 'unread-row' : ''}>
                    <td>
                      ...
                      <small className="text-slate-400" style={{ fontSize: '10px' }}>
                        {partageView === 'received' ? d.expediteur_ville : d.destinataire_ville}
                      </small>
                    </td>
                    <td>
                    ...
                    <td>
                      <div className="flex gap-2">
                        <button
                          ...
                          onClick={() => handleDownloadPartage(d.id)}
                          title="Télécharger"
                        >
                        ...
                        <button
                          ...
                          onClick={() => handleDeletePartage(d.id)}
                          title="Supprimer"
