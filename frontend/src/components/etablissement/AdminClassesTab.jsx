  ...
  return (
    <div className="classes-view">
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          <span
            ...
            style={{
              fontSize: '11.5px',
              padding: '6px 12px',
              background: 'rgba(19, 30, 108, 0.05)',
      ...
      <div className="classes-grid">
        {classes
          .filter((c) => c.annee_scolaire === classesAnneeFilter)
          .map((c) => {
              ...
              <div key={c.id} className="class-card animate-fade-in">
                ...
                {studentCount > 0 && (
                  <div className="class-info-body" style={{ marginTop: 0 }}>
                    ...
                    >
                      <span>
                        {garconCount} Garçon{garconCount > 1 ? 's' : ''}
                      </span>
                    </div>
                    <div
                      className="class-info-badge"
                ...
                <div className="class-card-footer">
                  <div className="flex gap-2 flex-wrap" style={{ width: '100%' }}>
                    ...
                    <button
                      className="btn-action-text"
                      title="Coefficients & Matières"
                      onClick={() => openCoefModal(c)}
                      style={{ flex: 1.2, justifyContent: 'center' }}
                    >
                      <BookMarked size={12} /> <span>Matières</span>
                    </button>
                    <button
                      className="btn-action-text link-btn"
                      title="Copier le lien d'inscription"
                      onClick={() => handleCopyRegLink(c.id)}
