      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            Bienvenue, {profile.nom || 'Administrateur'} — Code Établissement :{' '}
            <code>{profile.code_etablissement || 'LERAL-ETAB'}</code>
          </p>
        </div>
      ...
      <div className="mid-row">
        ...
        <div className="card">
          ...
          <div className="task-item">
            ...
            <div className="task-text">
              ...
              <div className="tcount">
                <span className="big-num">{classes.filter((c) => c.annee_scolaire === classesAnneeFilter).length}</span>{' '}
                classes
              </div>
            </div>
          </div>
          ...
          <div className="card-actions">
            <button className="btn btn-primary" onClick={() => navigate('/dashboard/eleves')} style={{ flex: 1 }}>
              Gérer les élèves
            </button>
          </div>
        </div>
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <div className="chart-semestre-toggle">
                <button className={`sm-btn ${chartSemestre === 1 ? 'active' : ''}`} onClick={() => setChartSemestre(1)}>
                  Semestre 1
                </button>
                <button className={`sm-btn ${chartSemestre === 2 ? 'active' : ''}`} onClick={() => setChartSemestre(2)}>
                  Semestre 2
                </button>
              </div>
            </div>
          ...
          {classAverages.length > 0 && (
            <div className="chart-footer">
              {(() => {
                ...
                return (
                  <span className="best-class">
                    🏆 Meilleure classe : <strong>{best.classe_nom}</strong> ({Number(best.moyenne_generale).toFixed(2)}
                    /20)
                  </span>
                );
              })()}
      ...
      <div className="table-container">
        <table className="data-table">
          ...
          <tbody>
              ...
              <tr key={e.id}>
                <td>
                  <div className="user-cell">
                    <div className="avatar av-blue" style={{ width: 24, height: 24, fontSize: 8 }}>
                      {e.prenom?.[0]}
                      {e.nom?.[0]}
                    </div>
            ...
            {eleves.length === 0 && (
              <tr>
                <td colSpan="4" className="text-center" style={{ padding: 20, color: 'var(--gray-400)' }}>
                  Aucun élève inscrit
                </td>
              </tr>
            )}
