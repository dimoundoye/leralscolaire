    ...
    <div className="notes-view">
      ...
      <div style={{ marginBottom: '20px' }}>
        <div className="status-toggle-pill" style={{ display: 'inline-flex' }}>
          <button
            className={`status-toggle-btn ${notesSubTab === 'saisie' ? 'active' : ''}`}
            onClick={() => setNotesSubTab('saisie')}
          >
            <BookOpenCheck size={14} style={{ marginRight: 6, verticalAlign: 'middle' }} /> Saisie des Notes
          </button>
          <button
            className={`status-toggle-btn ${notesSubTab === 'decisions' ? 'active' : ''}`}
            onClick={() => setNotesSubTab('decisions')}
          >
            ...
            <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
              ...
              <div className="input-group" style={{ margin: 0, flex: 1, minWidth: '180px' }}>
                ...
                >
                  ...
                  {classes
                    .filter((c) => c.annee_scolaire === notesAnneeScolaire)
                    .map((c) => (
              ...
              <div style={{ margin: 0 }}>
                <button
                  ...
                  disabled={loading || notesGrid.length === 0}
                  style={{
                  ...
                  >
                    ...
                    >
                      ...
                      {devoirCount < totalStudents ? (
                        ...
                        >
                          <span>⚠️</span> Il reste <strong>{totalStudents - devoirCount}</strong> élève(s) sans note de
                          devoir.
                        </p>
                      ) : (
                        <p
                    ...
                    >
                      ...
                      {examenCount < totalStudents ? (
                        ...
                        >
                          <span>⚠️</span> Il reste <strong>{totalStudents - examenCount}</strong> élève(s) sans note
                          d'examen.
                        </p>
                      ) : (
                        <p
                              ...
                              <tr key={n.eleve_id}>
                                ...
                                <td>
                                  <input
                                    ...
                                    className={'note-input' + (n.readonly ? ' note-readonly' : '')}
                                ...
                                <td>
                                  {n.note_devoir !== '' && n.note_examen !== '' ? (
                                    <span
                                      className={`status-pill ${moy >= 10 ? 'status-pass' : 'status-fail'}`}
                                      style={{ padding: '6px 12px', fontSize: '11px', fontWeight: 700 }}
                                    >
                                      {moy.toFixed(2)} / 20
                                    </span>
                                  ) : (
                                    <span className="text-slate-400" style={{ fontSize: '11px', fontWeight: 600 }}>
                                      Saisie incomplète
                                    </span>
                                  )}
                                </td>
                                <td>
                                  <input
                                    ...
                                    placeholder={
                                      n.readonly ? "Note d'un autre établissement" : 'Saisir appréciation...'
                                    }
                                    style={{
                                      width: '100%',
                                      height: '36px',
          ...
          >
            <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
              <div className="input-group" style={{ margin: 0, flex: 1, minWidth: '180px' }}>
                ...
                <select
                  value={decisionsClasseId}
                  onChange={(e) => {
                    setDecisionsClasseId(e.target.value);
                    if (e.target.value) fetchDecisions(e.target.value);
                  }}
                  className="pill-select"
                  style={{ width: '100%', borderRadius: '8px', padding: '8px 12px', height: '38px', fontSize: '12px' }}
                >
              ...
              <div style={{ margin: 0, display: 'flex', gap: 8 }}>
                <button
                  ...
                  onClick={() => setShowReglesModal(true)}
                    ...
                    {decisions.map((d) => {
                      ...
                      return (
                        <tr key={d.eleve_id}>
                          ...
                          <td>
                            <span
                              className={`status-pill ${moy >= 12 ? 'status-pass' : moy >= 10 ? 'status-warning' : 'status-fail'}`}
                              style={{ padding: '6px 12px', fontSize: '11px', fontWeight: 700 }}
                            >
                              {moy.toFixed(2)} / 20
                            </span>
                          ...
                          <td>
                            <select
                              value={d.decision === 'PASSAGE_DIRECT' ? 'PASSAGE' : d.decision}
                              onChange={(e) => updateDecision(d.eleve_id, 'decision', e.target.value)}
                              style={{
                                width: '100%',
                                padding: '6px 12px',
                                borderRadius: 8,
                          ...
                          <td>
                            {(() => {
                              const isCVOrPassage =
                                d.decision === 'PASSAGE' ||
                                d.decision === 'PASSAGE_DIRECT' ||
                                d.decision === 'COURS_VACANCES';
                              ...
                              const validValue = targetClasses.some((tc) => tc.nom === d.decision_detail)
                                ? d.decision_detail
                                : targetClasses
                                    .filter((c) => c.niveau === nextLevel)
                                    .find((tc) => tc.nom.endsWith(currentSuffix))?.nom ||
                              ...
                              if (targetClasses.length > 0) {
                                ...
                                return (
                                  <span
                                    className={`status-pill ${isRed ? 'status-fail' : 'status-pass'}`}
                                    style={{ padding: '6px 12px', fontSize: '11px', fontWeight: 700 }}
                                  >
                                    {nextLevel}
                                  </span>
        ...
        <div className="audit-view">
          ...
          >
            ...
            <p style={{ margin: 0, fontSize: '12px', fontWeight: 600, color: '#92400e' }}>
              Journal d'audit — Toutes les modifications de notes effectuées par les professeurs sont tracées de façon
          ...
          <div className="table-block">
            ...
            {auditLoading ? (
              ...
              <div style={{ padding: '40px 20px', textAlign: 'center' }}>
                ...
                <p style={{ color: 'var(--slate-400)', fontSize: '13px', fontWeight: 500 }}>
                  Aucune modification de note enregistrée.
                </p>
                <p style={{ color: 'var(--slate-300)', fontSize: '12px', marginTop: 4 }}>
                  Les modifications faites par les professeurs apparaîtront ici.
                </p>
              </div>
            ) : (
                        ...
                        <tr key={entry.id}>
                          <td>
                            ...
                            <div style={{ fontSize: '10px', color: 'var(--slate-400)', marginTop: 2 }}>
                              {new Date(entry.date_modification).toLocaleTimeString('fr-FR', {
                                hour: '2-digit',
                                minute: '2-digit',
                              })}
                            </div>
                          ...
                          <td>
                            <div className="user-cell">
                              ...
                              <span style={{ fontWeight: 600, fontSize: '12px' }}>
                                {entry.eleve_nom} {entry.eleve_prenom}
                              </span>
                            </div>
                          </td>
                          <td>
                            <span style={{ fontSize: '11px', fontWeight: 600, color: 'var(--slate-600)' }}>
                              {entry.classe_nom}
                            </span>
                          ...
                          <td>
                            <span
                              style={{
                                ...
                                textTransform: 'uppercase',
                                background:
                                  entry.type_note === 'DEVOIR' ? 'rgba(99,102,241,0.1)' : 'rgba(234,88,12,0.1)',
                                color: entry.type_note === 'DEVOIR' ? '#4f46e5' : '#c2410c',
                              }}
                            >
                              {entry.type_note === 'DEVOIR' ? 'Devoir' : 'Examen'}
                            </span>
                          </td>
                          <td style={{ textAlign: 'center' }}>
                            <span style={{ fontSize: '11px', fontWeight: 700, color: 'var(--slate-600)' }}>
                              S{entry.semestre || entry.trimestre || '1'}
                            </span>
                          </td>
                          <td style={{ textAlign: 'center' }}>
                            <span
                            ...
                            {entry.statut === 'EN_ATTENTE' ? (
                              <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                                <button
                                  ...
                                  onClick={() => openAuditConfirmModal(entry, 'confirm')}
                                >
                                  Confirmer
                                </button>
                                <button
                                  ...
                                  onClick={() => openAuditConfirmModal(entry, 'reject')}
                                >
                                  Rejeter
                                </button>
                              </div>
                              ...
                              <span
                                className="status-pill status-fail"
                                style={{ fontSize: '11px', fontWeight: 700, padding: '4px 10px' }}
                              >
                                Rejetée
                              </span>
        ...
        <div className="suivi-remplissage-view animate-fade-in">
          ...
          <div className="table-block">
            ...
            {suiviLoading ? (
              ...
              <div style={{ padding: '40px 20px', textAlign: 'center' }}>
                <p style={{ color: 'var(--slate-400)', fontSize: '13px', fontWeight: 500 }}>
                  Aucune classe trouvée dans l'établissement.
                </p>
              </div>
            ) : (
              <div className="table-container" style={{ border: 'none', boxShadow: 'none' }}>
                <table className="data-table">
                  ...
                  <tbody>
                    {suiviRemplissage.map((item) => {
                      const devPct =
                        item.expected_slots > 0 ? Math.round((item.filled_devoirs / item.expected_slots) * 100) : 0;
                      const examPct =
                        item.expected_slots > 0 ? Math.round((item.filled_examens / item.expected_slots) * 100) : 0;
                        ...
                        <tr key={item.classe_id}>
                          ...
                          <td>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                              ...
                              >
                                <span>
                                  {item.filled_devoirs} / {item.expected_slots}
                                </span>
                                <span>{devPct}%</span>
                              </div>
                              <div
                              ...
                              >
                                <div
                                  style={{ background: 'var(--primary-color)', width: `${devPct}%`, height: '100%' }}
                                />
                              </div>
                            </div>
                          </td>
                          <td>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                              ...
                              >
                                <span>
                                  {item.filled_examens} / {item.expected_slots}
                                </span>
                                <span>{examPct}%</span>
                              </div>
                              <div
                          ...
                          <td>
                            {isComplete ? (
                              <span className="status-pill status-pass" style={{ fontSize: '11px', fontWeight: 700 }}>
                                Complété
                              </span>
                            ) : (
                              <span className="status-pill status-fail" style={{ fontSize: '11px', fontWeight: 700 }}>
                                Incomplet
                              </span>
                            )}
                          </td>
                          <td style={{ textAlign: 'center' }}>
                            <button
                              ...
                              title={
                                item.bulletin_autorise
                                  ? 'Cliquer pour désactiver le téléchargement pour les élèves'
                                  : 'Cliquer pour autoriser le téléchargement pour les élèves'
                              }
                            >
                              {item.bulletin_autorise ? (
                                <>
