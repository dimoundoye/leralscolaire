...
const EmargementGeoSettings = () => {
  ...
  const capture = async (target) => {
    ...
    try {
      ...
      setMessage({
        type: 'info',
        text: `Position capturée (précision ± ${Math.round(pos.precision)} m). Vérifiez-la sur la carte avant d'enregistrer.`,
      });
    } catch (err) {
  ...
  const savePosition = async (e) => {
    ...
    try {
      ...
      setPosition({
        latitude: data.position.latitude,
        longitude: data.position.longitude,
        rayon_metres: data.position.rayon_emargement_metres,
      });
    ...
    >
      <div>
        ...
        <p style={{ fontSize: '12px', color: 'var(--slate-500)', margin: '6px 0 0' }}>
          Un professeur ne peut émarger un cours que s'il se trouve dans le rayon défini autour de l'établissement, ou
          autour du terrain d'EPS choisi pour une séance d'EPS.
        </p>
      </div>
      ...
      <form onSubmit={savePosition} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 120px', gap: '12px' }}>
          ...
          <div>
            <label style={labelStyle}>Rayon (m)</label>
            <input
              type="number"
              min="50"
              max="2000"
              required
              value={position.rayon_metres}
              onChange={(e) => setPosition({ ...position, rayon_metres: e.target.value })}
              style={inputStyle}
            />
          </div>
        </div>
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
          <button
            type="button"
            className="btn"
            onClick={() => capture('etablissement')}
            disabled={busy !== null}
            style={buttonStyle}
          >
            {busy === 'capture-etablissement' ? (
          ...
          <button
            ...
            disabled={busy !== null}
            style={{ ...buttonStyle, background: 'var(--primary-color)' }}
          >
            {busy === 'position' ? <Loader2 size={14} className="animate-spin" /> : 'Enregistrer la position'}
          </button>
          {hasPosition && (
            <a
              href={mapUrl(position.latitude, position.longitude)}
              target="_blank"
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        {terrains.length === 0 ? (
          <p style={{ fontSize: '12px', color: 'var(--slate-500)', margin: 0 }}>
            Aucun terrain enregistré : l'émargement EPS est impossible tant qu'aucun terrain n'est déclaré.
          </p>
        ) : (
          <ul
          ...
          >
            {terrains.map((terrain) => (
              ...
              >
                <span>
                  <strong>{terrain.nom}</strong> — rayon {terrain.rayon_metres} m
                </span>
                <span style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                  <a
                    href={mapUrl(terrain.latitude, terrain.longitude)}
                    target="_blank"
                    rel="noopener noreferrer"
                  ...
                  <button
                    ...
                    disabled={busy !== null}
                    title="Supprimer"
                    aria-label={`Supprimer ${terrain.nom}`}
                    style={{
                      background: 'none',
                      border: 'none',
                      color: '#b91c1c',
        ...
        >
          <div>
            <label style={labelStyle}>Nom du terrain</label>
            <input
              type="text"
              required
              maxLength={150}
              placeholder="Ex : Stade municipal de Ouakam"
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 120px', gap: '12px' }}>
            <div>
              ...
              <input
                ...
                onChange={(e) => setNewTerrain({ ...newTerrain, latitude: e.target.value })}
                style={inputStyle}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => setNewTerrain({ ...newTerrain, longitude: e.target.value })}
                style={inputStyle}
              />
            </div>
            <div>
              ...
              <input
                ...
                max="2000"
                required
                value={newTerrain.rayon_metres}
                onChange={(e) => setNewTerrain({ ...newTerrain, rayon_metres: e.target.value })}
                style={inputStyle}
              />
            </div>
          </div>
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            <button
              type="button"
              className="btn"
              onClick={() => capture('terrain')}
              disabled={busy !== null}
              style={buttonStyle}
            >
              {busy === 'capture-terrain' ? <Loader2 size={14} className="animate-spin" /> : <Crosshair size={14} />}{' '}
              Utiliser ma position actuelle
            </button>
            <button
              type="submit"
            ...
            >
              {busy === 'terrain' ? (
                ...
                <>
                  <Plus size={14} /> Ajouter le terrain
                </>
              )}
            </button>
          </div>
