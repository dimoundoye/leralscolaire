...
const AdminAttendanceTab = ({
  ...
  const demoProfsEmargements = [
    {
      id: 1,
      prof: 'Prof. Rassoul NDOYE',
      matiere: 'Mathématiques',
      classe: 'Terminale S2',
      heure: '08h03',
      mode: 'QR_SCAN_20S',
      statut: 'VALIDE_COMPLET',
      cahier: 'Chapitre 4: Intégration',
    },
    {
      id: 2,
      prof: 'Prof. Moussa DIOP',
      matiere: 'EPS',
      classe: '3ème A',
      heure: '08h05',
      mode: 'EPS_GPS_TERRAIN',
      statut: 'PRESENCE_SCANNEE',
      cahier: 'En attente',
    },
    {
      id: 3,
      ...
      heure: '09h00',
      mode: 'RATTRAPAGE',
      statut: 'RATTRAPAGE_APPROUVE',
      cahier: 'Rattrapage mécanique',
    },
    {
      id: 4,
      prof: 'Prof. Cheikh BA',
            ...
            <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
              ...
              <div className="input-group" style={{ margin: 0, flex: 1, minWidth: '150px' }}>
                ...
                >
                  ...
                  {classes
                    .filter((c) => c.annee_scolaire === attendanceAnneeFilter)
                    .map((c) => (
                      ...
                      <tr key={entry.id}>
                        <td>
                          <div style={{ fontSize: '11.5px', fontWeight: 700, color: 'var(--slate-700)' }}>
                            {new Date(entry.date_absence).toLocaleDateString('fr-FR', {
                              day: '2-digit',
                              month: 'short',
                              year: 'numeric',
                            })}
                        ...
                        <td>
                          <span style={{ fontSize: '11.5px', fontWeight: 600, color: 'var(--slate-600)' }}>
                            {entry.classe_nom}
                          </span>
                        </td>
                        <td>
                          <span
                            style={{
                              ...
                              textTransform: 'uppercase',
                              background:
                                entry.type_presence === 'ABSENCE' ? 'rgba(239,68,68,0.1)' : 'rgba(234,88,12,0.1)',
                        ...
                        <td>
                          {entry.motif ? (
                            <button
                              onClick={() =>
                                setViewMotifModal({ titre: 'Motif initial (Prof)', texte: entry.motif, entry })
                              }
                              style={{
                                padding: '3px 10px',
                                fontSize: '10.5px',
                        ...
                        <td>
                          {entry.motif_justification ? (
                            <button
                              onClick={() =>
                                setViewMotifModal({
                                  titre: 'Justification (Admin)',
                                  texte: entry.motif_justification,
                                  entry,
                                })
                              }
                              style={{
                        ...
                        <td style={{ textAlign: 'center' }}>
                          {!entry.justifiee ? (
                            ...
                            <span style={{ color: 'var(--slate-400)', fontSize: '11px', fontWeight: 600 }}>
                              Traité ✓
                            </span>
                          )}
                        </td>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
              ...
              <div>
                <h3 style={{ margin: 0, fontSize: '18px', fontWeight: 900, color: '#ffffff' }}>
                  Borne d'Émargement Sécurisée TOTP 20s
                </h3>
                <p style={{ margin: '4px 0 0', fontSize: '12px', color: '#94a3b8' }}>
                  Affichez le QR Code dynamique 20s en plein écran sur la télé de la salle des profs ou un second écran
                  sans bloquer votre travail.
                </p>
              </div>
            </div>
                    ...
                    <tr key={item.id}>
                      ...
                      <td>
                        <div style={{ fontSize: '12px', fontWeight: 600 }}>
                          {item.matiere} • <span style={{ color: 'var(--slate-500)' }}>{item.classe}</span>
                        </div>
                      </td>
                      <td>
                      ...
                      <td>
                        <span
                          style={{
                            fontSize: '11px',
                            color: item.cahier.includes('En attente') ? '#d97706' : '#166534',
                            fontWeight: 600,
                          }}
                        >
                          {item.cahier}
                      ...
                      <td>
                        {item.statut === 'VALIDE_COMPLET' && (
                          <span style={{ fontSize: '11px', fontWeight: 700, color: '#16a34a' }}>Validé complet</span>
                        )}
                        {item.statut === 'PRESENCE_SCANNEE' && (
                          <span style={{ fontSize: '11px', fontWeight: 700, color: '#d97706' }}>Présence scannée</span>
                        )}
                        {item.statut === 'RATTRAPAGE_APPROUVE' && (
                          <span style={{ fontSize: '11px', fontWeight: 700, color: '#2563eb' }}>
                            Rattrapage approuvé
                          </span>
                        )}
                        {item.statut === 'ABSENT_NON_EMARGE' && (
                          <span style={{ fontSize: '11px', fontWeight: 700, color: '#dc2626' }}>Non émargé</span>
                        )}
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        {item.statut === 'ABSENT_NON_EMARGE' ? (
